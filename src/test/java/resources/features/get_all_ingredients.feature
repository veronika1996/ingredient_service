Feature: Get all ingredients
  ...
  Scenario: Successfully retrieve all ingredients when list is empty
    ...
    When I send a GET request to "/ingredients?username=User0"
    Then I should receive a 200 response
    And the response should contain an empty list

  @getAll
  Scenario: Successfully retrieve all ingredients when user doesn't have ingredients
    Given the ingredient list contains 1 ingredient
    When I send a GET request to "/ingredients?username=User0"
    Then I should receive a 200 response
    And the response should contain an empty list

  @getAll
  Scenario Outline: Successfully retrieve all ingredients when user has <number> ingredient
    Given the ingredient list contains <number> ingredient
    When I send a GET request to "/ingredients?username=User1"
    Then I should receive a 200 response
